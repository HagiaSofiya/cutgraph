import type { Graph, GraphEdge, GraphNode, MediaRef, NodeStatus, NodeType } from '../src/types';
...
export function makeEdge(overrides: Partial<GraphEdge> & { id: string; source: string; target: string }): GraphEdge {
  return {
    id: overrides.id,
...
export function statusOf(graph: Graph, nodeId: string): NodeStatus {
